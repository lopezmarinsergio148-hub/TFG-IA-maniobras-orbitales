...
from poliastro.bodies import Sun, Mars, Earth, Venus, Jupiter
...
"""
Para estos ejercicios veremos como realizar maniobras para poder viajar entre los diferentes planetas.
"""
...
def transferencia_lambert(days=150):
...
def mision_marte_real():
    ...
    earth_ephem = Ephem.from_body(Earth, t_span) #Con esto puede saber donde estará la Tierra durante todo t_span.
    mars_ephem = Ephem.from_body(Mars, t_span)

    # 3. Posiciones y velocidades exactas interpoladas
    r_ini, v_ini_earth = earth_ephem.rv(fecha_lanzamiento) #Con el rv conseguimos sacar la posición y la velocidad
    r_dest, v_mars_arrival = mars_ephem.rv(fecha_llegada)

    # 4. Resolver Lambert
    it = lambert(Sun.k, r_ini, r_dest, tiempo_vuelo) #Conjunto de vectores de velocidad necesarios para que la órbita sea posible.
...
def porkchop_marte():
    ...
    dv_matrix = np.full((len(duraciones), len(lanzamientos)), np.nan) #Crea una matriz y la rellena con nan
    ...
    idx = np.unravel_index(np.nanargmin(dv_matrix), dv_matrix.shape) #Con el np.nanargmin(dv_matrix) buscamos el número más pequeño y además ignora los huecos vacíos (NaN)
    #Con np.unravel_index(..., dv_matrix.shape) conseguimos que lo que nos da antes que, que es como una matriz de una sola fila, la transforma a coordenadas (fila,columna)
    dv_min = dv_matrix[idx] #Guarda el valor
    ...
    # (Sin título embebido: en la memoria la información va en el pie de figura \caption{}.
    #  Para uso interactivo puedes descomentar la línea siguiente.)
    ...
    # Un tick por mes con el año completo: evita las etiquetas repetidas ("Sep 26 / Sep 26 ...")
    ax.xaxis.set_major_locator(mdates.MonthLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%b %Y'))
    fig.autofmt_xdate(rotation=30)  # rota las fechas para que no se solapen

    plt.legend()
    plt.grid(True, alpha=0.1)
    plt.tight_layout()
    # Exportar para la memoria: PDF vectorial, ruta sin tildes ni espacios
    plt.savefig("imagenes/porkchop_marte.pdf", bbox_inches="tight")
    plt.show()
...
def crear_animacion_marte():
    ...
    v_trans_ini, v_trans_arrival = it[0] if isinstance(it, list) else it
    trans_orb = Orbit.from_vectors(Sun, r_ini, v_trans_ini, epoch=fecha_lanzamiento) #Con el epoch le decimos cuando empieza
...
if __name__ == "__main__":
    #hohmann_tierra_marte()
    #transferencia_lambert(days=150)
    #mision_marte_real()
    porkchop_marte()
    #crear_animacion_marte()
    #animacion_tierra_venus()
    #porkchop_venus()
    #porkchop_jupiter()
    #animacion_jupiter()